...
CREATE TABLE reading_activity_logs (
    ...
    -- untuk nentuin warna heatmap (misal: jumlah halaman yang dibaca hari itu)
    date DATE NOT NULL,
    UNIQUE (user_id, date)
);
CREATE TABLE conversations (
    ...
    -- untuk menyimpan waktu pesan terakhir dikirim, berguna untuk mengurutkan percakapan di UI
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX idx_notifications_unread ON notifications(user_receiver_id)
WHERE is_read = FALSE;
