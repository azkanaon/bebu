  ...
  useEffect(() => {
		try {
			...
			if (authStorage) {
				...
				if (parsedStorage?.state?.user?.status === "suspended") {
					setIsSuspended(true);
				}
			}
		} catch (error) {
			console.error("Gagal membaca status auth:", error);
		}
  }, []);
						...
						<AnimatePresence>
							{showOptions && (
								<motion.div
									...
									className="absolute right-0 mt-2 w-52 bg-[#0B1220] border border-white/10 rounded-xl shadow-2xl overflow-hidden py-1"
								>
									{/* 🔥 TOMBOL BLOCK USER */}
									<button
									...
									>
										...
										{isSuspended && (
											<span className="text-red-400/40 text-xs">
												🔒
											</span>
										)}
									</button>
									...
									<button
										...
										className={`w-full px-4 py-2.5 text-left text-sm flex items-center justify-between transition-colors ${
											isSuspended
												? "text-red-400/50 bg-red-950/10 cursor-not-allowed"
												: "text-gray-400 hover:bg-white/5 hover:text-red-400 cursor-pointer"
										}`}
									>
										<div className="flex items-center gap-3">
											<Flag size={16} />
