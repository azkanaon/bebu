...
import { useInfiniteBadges } from '@/api/profile/useInfiniteCollection'
...
export default function BadgesOverlay({
  ...
  const filteredItems = useMemo(() => {
    return allItems.filter((item) =>
      item.badgeName.toLowerCase().includes(search.toLowerCase()),
    )
  }, [allItems, search])
              ...
              <div className="p-6 border-b border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h2 className="text-2xl font-bold flex items-center gap-2 italic uppercase tracking-tighter">
                    <Medal className="text-amber-500" /> User Badges
                  </h2>
                  <p className="text-[10px] text-gray-500 uppercase font-bold tracking-widest mt-1">
                    {data?.pages[0]?.meta.totalItems || 0} Badges Collected
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="relative flex-1 md:w-64">
                    ...
                    <input
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl py-2 pl-9 pr-4 text-xs outline-none focus:border-amber-500/50"
                    />
                  </div>
                  <button
                    onClick={onClose}
                ...
                {isLoading ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
                    {filteredItems.map((item, i) => {
                      const vibe = getBadgeVibe(item.badgeName)
                      const isStarred = favIds.includes(item.badgeId)
                      ...
                      return (
                        <motion.div
                          ...
                          whileHover={{ y: -8 }} // Stabil: Hanya naik sedikit ke atas
                        ...
                        >
                          ...
                          <div className="relative mb-5">
                            ...
                            <div
                              className={`relative w-20 h-20 rounded-full bg-linear-to-br ${vibe.gradient} to-transparent flex items-center justify-center p-3 border border-white/10 shadow-inner transition-transform duration-500 group-hover:scale-110`}
                            >
                              <Image
                                src={item.logoUrl}
                                alt={item.badgeName}
                          ...
                          {/* SHINE STREAK (Sama seperti Achievement - Stabil & Smooth) */}
