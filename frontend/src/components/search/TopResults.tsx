				...
				<section className="space-y-4">
					...
					<div className="flex gap-3 overflow-x-auto no-scrollbar pb-2 px-1">
						{users.map((user) => (
							...
							>
								<div className="relative w-14 h-14 rounded-full border-2 border-blue-500/20 bg-slate-800">
									<UserAvatar
										user={{
											avatar_url: user.avatarUrl,
											display_name: user.username,
										}}
										size={56} // Menggantikan w-14 h-14 (14 * 4px = 56px)
				...
				<section className="space-y-4">
					...
					<div className="grid grid-cols-2 gap-4">
						{books.map((book) => (
							...
							>
								<div className="relative w-16 h-22 rounded-lg shrink-0 shadow-lg bg-slate-800 border border-white/10">
									<BookCover
										src={book.cover_img_url}
										title={book.title}
										width={64} // Menggantikan w-16 (16 * 4px = 64px)
										height={88} // Menggantikan h-22 (22 * 4px = 88px)
