...
import ClientPortal from "@/components/ClientPortal"; // Pastikan import ClientPortal tersedia
...
export default function ReportDetailModal({
	...
	const requiresDuration = (action: string) => {
		return ["suspend", "shadowban_user", "shadowban_post"].includes(action);
	};
	...
	return (
		<>
			...
			>
				...
				>
					{/* Ambient */}
					<div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(59,130,246,0.08),transparent_40%)]" />

					{/* HEADER */}
					<div className="relative flex items-start justify-between border-b border-white/5 px-6 py-5">
						<div>
							<div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-red-300/80">
								<ShieldAlert size={12} />
								Trust & Safety
							</div>
					...
					{/* CONTENT */}
					<div className="custom-scrollbar max-h-[68vh] overflow-y-auto p-6">
						{loading ? (
							...
							<div className="space-y-6">
									...
									{detail.entity_type === "user" &&
										detail.user_data && (
											<a
												...
												className="group/report-user block overflow-hidden from-white/[0.04] to-white/[0.02] transition-all duration-200"
											>
												<div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
													{/* LEFT */}
													<div className="flex min-w-0 gap-3.5">
														{/* AVATAR */}
														<div className="h-[52px] w-[52px] shrink-0 overflow-hidden rounded-full ring-1 ring-white/10 bg-white/[0.03]">
															<UserAvatar
																user={{
																	avatar_url:
																		detail
														...
														<div className="min-w-0">
															...
															<div className="mt-0.5 flex items-center gap-1.5">
																<h3 className="truncate text-base font-semibold tracking-tight text-white transition-all duration-200 group-hover/report-user:text-blue-100 group-hover/report-user:drop-shadow-[0_0_10px_rgba(96,165,250,0.18)]">
																	{
																		detail
																			.user_data
																			.display_name
																...
																<ExternalLink
																	size={13}
																	className="shrink-0 text-zinc-600 transition-all duration-200 group-hover/report-user:text-blue-400 group-hover/report-user:translate-x-[1px] group-hover/report-user:-translate-y-[1px]"
																/>
															</div>
															...
															{/* INLINE STATS */}
															<div className="mt-2 grid grid-cols-3 gap-x-3 gap-y-1.5 sm:flex sm:items-center sm:gap-x-2.5 text-xs text-zinc-400 bg-white/[0.02] border border-white/5 w-full sm:w-fit px-3 py-1.5 rounded-lg">
																<InlineStat
																	icon={
																		<Users
																			size={
																...
																/>
																<span className="hidden sm:inline text-zinc-700 text-[10px] shrink-0">
																	•
																</span>
																<InlineStat
																	icon={
																...
																/>
																{/* Hot Score ditaruh di baris baru otomatis pada mobile, atau berjejer di desktop */}
																<div className="col-span-3 sm:hidden border-t border-white/5 my-0.5" />{" "}
																{/* Garis pembatas tipis khusus mobile */}
																<InlineStat
																	icon={
																		<Flame
																			size={
																	...
																	value={formatCompactNumber(
																		Number(
																			detail.user_data.hot_score?.toFixed(
																				1,
																			),
																		) || 0,
																	)}
											...
											<div className="space-y-4">
												...
												>
													<div
														...
														rounded-2xl bg-purple-500/[0.10] text-purple-300
												...
												<div className="space-y-4 pl-0">
													...
													{/* IMPLEMENTASI BLUR REFLECTION IMAGE BOX */}
													{detail.post_data
														.img_url && (
														<motion.div
															whileHover={{
															...
															className="
															relative w-full h-[260px] overflow-hidden rounded-2xl
															border border-white/10 bg-zinc-950
															cursor-zoom-in group
														"
														>
															{/* Lapisan Background: Zoom & Blur */}
															<img
																src={
																	detail
																		.post_data
								...
								<div className="space-y-3">
									...
									<div className="flex flex-wrap gap-2">
										{detail.reason_counts.map((rc, idx) => (
											<div
												key={idx}
												className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-1.5"
											>
												<span className="text-[11px] text-zinc-300">
													{rc.reason_text}
												</span>
												<div className="rounded-full bg-red-500/[0.10] px-2 py-0.5 text-[10px] font-semibold text-red-300">
													{rc.count}
												</div>
											</div>
										))}
										...
										<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
											...
											<div className="flex items-center gap-4">
												<input
													...
													className="h-10 w-20 rounded-2xl border border-white/10 bg-black/20 px-3 text-sm text-white outline-none focus:border-blue-400/30"
												/>
								...
								{detail.moderation_history && (
									<div className="relative overflow-hidden rounded-[22px] border border-emerald-500/10 bg-emerald-500/[0.04] p-4">
										...
										{detail.moderation_history.reason && (
											<div className="relative mt-3 rounded-2xl border border-white/5 bg-black/20 p-3">
								...
								{!isLocked && (
									<div className="space-y-3">
										...
										<textarea
											...
											className="custom-scrollbar overflow-y-auto w-full rounded-[24px] border border-white/10 bg-white/[0.03] p-4 text-sm text-zinc-200 placeholder:text-zinc-600 outline-none transition-all duration-200 focus:border-blue-400/30 focus:bg-white/[0.05] resize-none"
										/>
									</div>
								)}
							</div>
					...
					<div className="flex items-center justify-between border-t border-white/5 bg-black/20 px-6 py-4">
						...
						<div className="flex items-center gap-3">
							<button
								onClick={onClose}
								className="h-10 rounded-2xl border border-white/10 bg-white/[0.03] px-4 text-sm font-medium text-zinc-300 transition-all duration-200 hover:border-white/20 hover:text-white"
							>
								Cancel
							</button>
							...
							<button
								...
								disabled={
									submitting || !selectedAction || isLocked
								}
								className="h-10 rounded-2xl bg-red-600 px-4 text-sm font-semibold text-white transition-all duration-200 hover:bg-red-500 disabled:pointer-events-none disabled:opacity-40"
							>
								{submitting
									? "Processing..."
									: "Execute Action"}
			...
			{/* PORTAL MODAL VIEW UNTUK FULLSCREEN PREVIEW IMAGE */}
			<AnimatePresence>
				{isImageOpen && detail?.post_data?.img_url && (
					<ClientPortal>
						<motion.div
							initial={{ opacity: 0 }}
							animate={{ opacity: 1 }}
						...
						>
							...
							<div className="relative z-10 w-screen h-screen flex items-center justify-center">
								<motion.img
									...
									src={detail.post_data.img_url}
									alt="full screen content review"
...
function TimelineCard({
	...
	return (
		<div className="flex items-start gap-3 rounded-[24px] border border-white/10 bg-white/[0.03] p-4">
			<div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-blue-500/[0.10] text-blue-300">
				{icon}
			</div>
			<div>
