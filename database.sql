-- ============================================================
--  PostgreSQL Database Schema — Dual-Column ID Approach
--  Internal: SERIAL (fast joins & FKs)
--  External: UUID  (safe to expose in APIs & URLs)
-- ============================================================
...
CREATE TABLE user_follows (
    ...
    -- Junction table: no public_id needed, not directly exposed via API
);
...
CREATE TABLE achievements (
    ...
    description      TEXT,
    logo_url         TEXT,
    criteria_value   INT,  -- nilai yang diperlukan untuk mencapai pencapaian ini (misalnya, 1000 posts, 5000 likes, dll.)
    total_exp        INT         NOT NULL DEFAULT 0,
    created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- ============================================================
-- READING: BOOKSHELVES, WRAPS, ACTIVITY
-- ============================================================
...
-- public_id lookups (API layer always queries by these)
...
CREATE INDEX idx_post_comments_public_id     ON post_comments(public_id);
...
-- Internal FK traversal (DB layer uses these for joins)
